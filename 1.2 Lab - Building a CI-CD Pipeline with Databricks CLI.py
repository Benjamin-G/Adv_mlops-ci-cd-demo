...
def setup_git_repo(config_path="var/git_credentials.cfg"):
    ...
    try:
        ...
        subprocess.run('git config --global user.name "Benjamin-G"', shell=True, check=True)
        subprocess.run(f'git config --global user.email "{github_username}"', shell=True, check=True)
...
dbutils.widgets.text("subsample", "0.8", "Subsample Fraction")

## Retrieve Widget Values
max_depth = dbutils.widgets.get("max_depth")
n_estimators = dbutils.widgets.get("n_estimators")
subsample = dbutils.widgets.get("subsample")
...
def commit_and_push_changes():
    try:
        ...
        if result.strip():  # If there are changes to commit
            subprocess.run(["git", "commit", "-m", "Update pipeline configuration and version"], check=True)
